...
def rezervasyon_olustur():
    ...
    try:
        ...
        cursor.execute("INSERT INTO REZERVASYON (Tarih, Kisi_Sayisi, Musteri_Id, Masa_Id) VALUES (%s, %s, %s, %s)", 
                       (tarih_saat_str, kisi_sayisi, musteri_id, masa_id))
        db.commit()
...
def musteri_sil(id):
    ...
    try:
        ...
        db.commit()
        flash("Müşteri sistemden silindi!", "success")
    except Exception as e:
        db.rollback()
        flash(f"Hata: {str(e)}", "danger")
    finally:
...
def masalar_listesi():
    ...
    cursor.close()
    db.close()
    return render_template('masalar.html', masalar=tum_masalar)
...
def menu_listesi():
    ...
    cursor.close()
    db.close()
    return render_template('menu.html', urunler=tum_urunler)
...
def urun_ekle():
    ...
    try:
        ...
        db.commit()
        flash(f"'{ad}' menüye eklendi!", "success")
    except Exception as e:
        db.rollback()
    finally:
        db.close()
...
# ================= 5. SİPARİŞLER (ADİSYONLAR) =================
@app.route('/siparisler')
def siparis_listesi():
    db = get_db()
    cursor = db.cursor(dictionary=True)
    cursor.execute("SELECT s.Siparis_Id, s.Masa_Id, s.Tutar, m.Konum FROM SIPARIS s JOIN MASA m ON s.Masa_Id = m.Masa_Id")
    siparisler = cursor.fetchall()
    
    # EKSİK OLAN ÜRÜNLER LİSTESİ EKLENDİ (Dropdown için)
    ...
    cursor.close()
    db.close()
    return render_template('siparisler.html', siparisler=siparisler, urunler=urunler)

@app.route('/adisyon-urun-ekle/<int:siparis_id>', methods=['POST'])
def adisyon_urun_ekle(siparis_id):
    ...
    try:
        ...
        cursor.execute("INSERT INTO SIPARIS_DETAY (Siparis_Id, Urun_Id, Adet, Ara_Toplam) VALUES (%s, %s, %s, %s)", 
                       (siparis_id, urun_id, adet, ara_toplam))
        cursor.execute("UPDATE SIPARIS SET Tutar = (SELECT SUM(Ara_Toplam) FROM SIPARIS_DETAY WHERE Siparis_Id = %s) WHERE Siparis_Id = %s", (siparis_id, siparis_id))
        db.commit()
    finally:
        db.close()
...
def siparis_detay_sil(detay_id):
    ...
    cursor = db.cursor(dictionary=True)
    try:
        cursor.execute("SELECT Siparis_Id FROM SIPARIS_DETAY WHERE Detay_Id = %s", (detay_id,))
        sip_id = cursor.fetchone()['Siparis_Id']
        cursor.execute("DELETE FROM SIPARIS_DETAY WHERE Detay_Id = %s", (detay_id,))
        cursor.execute("UPDATE SIPARIS SET Tutar = (SELECT COALESCE(SUM(Ara_Toplam), 0) FROM SIPARIS_DETAY WHERE Siparis_Id = %s) WHERE Siparis_Id = %s", (sip_id, sip_id))
        db.commit()
    finally:
        db.close()
    return redirect(url_for('siparis_listesi'))
